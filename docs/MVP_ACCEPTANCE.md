...
| ------------------------------ | ---------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Full engineering gate          | `make check`                                   | Formatting, Clippy, Rust and frontend tests, TypeScript check, frontend build, browser E2E, and CLI acceptance all pass.                                                     |
...
| Desktop create/read/import/query/Lint/History/Restore loop | ✅     | `e2e/desktop.spec.ts` drives the React application with Playwright; deterministic data is supplied by the browser-only backend. Tauri IPC itself remains covered by Rust command/engine tests rather than a WebKit browser harness. |
